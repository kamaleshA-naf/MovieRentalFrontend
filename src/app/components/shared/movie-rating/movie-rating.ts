...
@Component({
  ...
  template: `
    <div class="rating-wrap">
      <button class="rate-trigger"
        ...
        [class.rate-locked]="!canRate"
        [title]="!canRate ? 'Rent this movie to rate it' : ''">
        @if (currentRating() === 1) {
          ...
          <svg width="18" height="18" viewBox="0 0 24 24"
            fill="currentColor" [attr.opacity]="canRate ? '0.7' : '0.3'">
            <path d="M1 21h4V9H1v12zm22-11c0-1.1-.9-2-2-2h-6.31l.95-4.57.03-.32c0-.41-.17-.79-.44-1.06L14.17 1 7.59 7.59C7.22 7.95 7 8.45 7 9v10c0 1.1.9 2 2 2h9c.83 0 1.54-.5 1.84-1.22l3.02-7.05c.09-.23.14-.47.14-.73v-2z"/>
          </svg>
          <span class="r-text">{{ canRate ? 'Rate' : 'Rent to Rate' }}</span>
        }
      </button>
  ...
  styles: [`
    ...
    .rate-trigger.rate-locked {
      opacity: 0.4;
      cursor: not-allowed;
      border-color: rgba(255,255,255,0.06);
    }
    .rate-trigger.rate-locked:hover {
      background: rgba(255,255,255,0.07);
      transform: none;
    }
    .rate-trigger.rate-locked .r-text {
...
export class MovieRatingComponent implements OnInit {
  ...
  /** Pass true when user has an Active or Expired rental for this movie */
  @Input() canRate = false;
  ...
  toggle(): void {
    if (!this.canRate) {
      this.toastr.warning(
        'You can rate this movie only after renting and watching it.',
        'Rental Required',
  ...
  rate(value: number): void {
    ...
    // Only send ratingValue — userId comes from JWT on backend
    this.ratingSvc.rateMovie(this.movieId, { ratingValue: value }).subscribe({
      next: (res) => {
