...
export class CartService {
  ...
  loadCart(userId: number): void {
    this.http
      ...
      .pipe(catchError((err) => {
        // 404 = no cart yet or endpoint not available — treat as empty, don't log
        if (err?.status !== 404) {
  ...
  checkout(userId: number, paymentMethod: string = 'UPI'): Observable<any> {
    return this.http
      .post<any>(`${this.API}/Cart/checkout`, { userId, paymentMethod })
      .pipe(catchError(err => { throw err; }));
  }
